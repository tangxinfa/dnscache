...
var CacheObject = function (conf) {
    ...
    conf.keepalive = parseInt(conf.keepalive, 10) || 100; //0 is not permissible
    conf.cachesize = parseInt(conf.cachesize, 10) || 1000; //0 is not permissible
    
    this.ttl = conf.ttl * 1000;
    this.keepalive = conf.keepalive * 1000;
    ...
    this.get = function (key, callback) {
        ...
        next(function () {
            ...
            if (conf.ttl !== 0 && (Date.now() - self.data[key].ts) >= self.ttl) {
                ...
                keepalive = (conf.keepalive !== 0 && (Date.now() - self.data[key].ts) >= self.keepalive);
            }
            callback(null, value, keepalive);
        });
    };
};
