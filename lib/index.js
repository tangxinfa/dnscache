...
var EnhanceDns = function (conf) {
        ...
        dns.lookup = function (domain, family, callback) {
            ...
            cache.get('lookup_' + domain + '_' + family, function (error, record, stale) {
                if (record) {
        ...
        dns.resolve = function (domain, type, callback) {
            ...
            cache.get('resolve_' + domain + '_' + type_new, function (error, record, stale) {
                if (record) {
        ...
        dns.resolve4 = function (domain, callback) {
            cache.get('resolve4_' + domain, function (error, record, stale) {
                if (record) {
        ...
        dns.resolve6 = function (domain, callback) {
            cache.get('resolve6_' + domain, function (error, record, stale) {
                if (record) {
                    callback(error, deepCopy(record));
                    if (! stale) {
                        return;
        ...
        dns.resolveMx = function (domain, callback) {
            cache.get('resolveMx_' + domain, function (error, record, stale) {
                if (record) {
                    callback(error, deepCopy(record));
                    if (! stale) {
                        return;
        ...
        dns.resolveTxt = function (domain, callback) {
            cache.get('resolveTxt_' + domain, function (error, record, stale) {
                if (record) {
                    callback(error, deepCopy(record));
                    if (! stale) {
                        return;
        ...
        dns.resolveSrv = function (domain, callback) {
            cache.get('resolveSrv_' + domain, function (error, record, stale) {
                if (record) {
                    callback(error, deepCopy(record));
                    if (! stale) {
                        return;
        ...
        dns.resolveNs = function (domain, callback) {
            cache.get('resolveNs_' + domain, function (error, record, stale) {
                if (record) {
                    callback(error, deepCopy(record));
                    if (! stale) {
                        return;
        ...
        dns.resolveCname = function (domain, callback) {
            cache.get('resolveCname_' + domain, function (error, record, stale) {
                if (record) {
                    callback(error, deepCopy(record));
                    if (! stale) {
                        return;
        ...
        dns.reverse = function (ip, callback) {
            cache.get('reverse_' + ip, function (error, record, stale) {
                if (record) {
                    callback(error, deepCopy(record));
                    if (! stale) {
                        return;
