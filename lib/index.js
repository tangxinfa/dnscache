...
var EnhanceDns = function (conf) {
        ...
        conf.keepalive = parseInt(conf.keepalive, 10) || 100; //0 is not allowed ie it keepalive is set to 0, it will take the default
        ...
        dns.lookup = function (domain, options, callback) {
            ...
            cache.get('lookup_' + domain + '_' + family + '_' + hints + '_' + all, function (error, record, keepalive) {
                if (record) {
        ...
        dns.resolve = function (domain, type, callback) {
            ...
            cache.get('resolve_' + domain + '_' + type_new, function (error, record, keepalive) {
                if (record) {
        ...
        dns.resolve4 = function (domain, callback) {
            cache.get('resolve4_' + domain, function (error, record, keepalive) {
                if (record) {
        ...
        dns.resolve6 = function (domain, callback) {
            cache.get('resolve6_' + domain, function (error, record, keepalive) {
                if (record) {
                    callback(error, deepCopy(record));
                    if (! keepalive) {
                        return;
        ...
        dns.resolveMx = function (domain, callback) {
            cache.get('resolveMx_' + domain, function (error, record, keepalive) {
                if (record) {
                    callback(error, deepCopy(record));
                    if (! keepalive) {
                        return;
        ...
        dns.resolveTxt = function (domain, callback) {
            cache.get('resolveTxt_' + domain, function (error, record, keepalive) {
                if (record) {
                    callback(error, deepCopy(record));
                    if (! keepalive) {
                        return;
        ...
        dns.resolveSrv = function (domain, callback) {
            cache.get('resolveSrv_' + domain, function (error, record, keepalive) {
                if (record) {
                    callback(error, deepCopy(record));
                    if (! keepalive) {
                        return;
        ...
        dns.resolveNs = function (domain, callback) {
            cache.get('resolveNs_' + domain, function (error, record, keepalive) {
                if (record) {
                    callback(error, deepCopy(record));
                    if (! keepalive) {
                        return;
        ...
        dns.resolveCname = function (domain, callback) {
            cache.get('resolveCname_' + domain, function (error, record, keepalive) {
                if (record) {
                    callback(error, deepCopy(record));
                    if (! keepalive) {
                        return;
        ...
        dns.reverse = function (ip, callback) {
            cache.get('reverse_' + ip, function (error, record, keepalive) {
                if (record) {
                    callback(error, deepCopy(record));
                    if (! keepalive) {
                        return;
